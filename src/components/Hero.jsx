...
const Hero = () => {
  ...
  return (
    <div>
      <div
        className="max-w-[1440px] my-0 mx-auto h-[600px] relative flex
            overflow-hidden"
      >
        <div
          className="flex flex-nowrap w-full h-full bg-red-800 "
          ref={slideCarrusel}
        >
          {imgsprueba?.length > 0 &&
            imgsprueba.map((item, index) => {
              return (
                <img
                  key={index}
                  src={item.img}
        ...
        {/* Left Arrow */}
        <button
          className="opacity-50 hover:opacity-100 cursor-pointer absolute top-0 bottom-0 m-auto bg-[#515151b3] w-[30px] h-[72px] flex items-center justify-center
                z-10"
          onClick={leftClick}
        >
      ...
      <div className="max-w-6xl my-0  mx-auto flex  items-center justify-center mt-[10px] gap-3 ">
        ...
        <button
          className={`w-[15px] h-[15px] rounded-[50%] bg-gray-400  hover:cursor-pointer`}
        ></button>
      </div>
    </div>
  );
